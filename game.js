...
// try loading a default mandible sprite - put your attached image at this path if running locally
...
function drawMobs(){
    mobs.forEach(mob=>{
        ...
        if(mob && mob.segments && Array.isArray(mob.segments) && mob.segments.length){
            ...
            const barHeight = 8; const bx = Math.round(head.x - barWidth/2); const by = Math.round(head.y - head.radius - 14);
            ...
            if(mob.projectiles && mob.projectiles.length){ mob.projectiles.forEach(p=>{ ctx.fillStyle=(p.type==='Missile')?'grey':'#f7d86b'; ctx.beginPath(); ctx.arc(p.x,p.y,p.radius,0,Math.PI*2); ctx.fill(); }); }
            return;
        }
        ...
        if(mob.spriteKey && window.MOB_SPRITES && window.MOB_SPRITES[mob.spriteKey] && window.MOB_SPRITES[mob.spriteKey].loaded){
            try{
                ...
                const flip = (mob.vx && mob.vx < -0.2) || (mob._patrolDir && mob._patrolDir < 0);
                ctx.translate(mob.x, mob.y);
                if(flip) ctx.scale(-1,1);
                ctx.drawImage(s.canvas, -drawW/2, -drawH/2, drawW, drawH);
                ctx.restore();

                // mandible animation for 'mandible' key
                if(mob.spriteKey === 'mandible'){
                    const phase = (mob._mandiblePhase || 0) + (Date.now()/160);
                    const open = Math.abs(Math.sin(phase)) * (Math.max(4, mob.radius*0.35));
                    const frontX = mob.x + (flip ? -mob.radius*0.5 : mob.radius*0.5);
                    const frontY = mob.y + (mob.radius*0.06);
                    ctx.fillStyle = '#1f1f1f';
                    ctx.beginPath(); ctx.moveTo(frontX, frontY); ctx.lineTo(frontX + (flip?-open:open), frontY - open*0.6); ctx.lineTo(frontX + (flip?-open:open), frontY + open*0.6); ctx.closePath(); ctx.fill();
                    ctx.beginPath(); ctx.moveTo(frontX - (flip?6:-6), frontY); ctx.lineTo(frontX - (flip?6:-6) - (flip?-open:open), frontY - open*0.6); ctx.lineTo(frontX - (flip?6:-6) - (flip?-open:open), frontY + open*0.6); ctx.closePath(); ctx.fill();
                }
                return;
            }catch(e){}
        }
        ...
        if((mob.type||'').toLowerCase() === 'hornet'){
            const flipped = mob._turnUntil && mob._turnUntil > Date.now();
            ctx.save(); ctx.translate(mob.x, mob.y);
            if(flipped) ctx.rotate(Math.PI);
            // outer rim (golden ring)
            ctx.fillStyle = '#c89b3a'; ctx.beginPath(); ctx.ellipse(0,0,mob.radius*1.18,mob.radius*0.95,0,0,Math.PI*2); ctx.fill();
            // inner body (lighter yellow)
            ctx.fillStyle = '#ffd36b'; ctx.beginPath(); ctx.ellipse(0,0,mob.radius*0.95,mob.radius*0.72,0,0,Math.PI*2); ctx.fill();
            // refined black stripes (slim, rotated)
            ctx.save(); ctx.rotate(-0.12);
            ctx.fillStyle = '#1b1b1b';
            for(let i=-2;i<=2;i++){
                const ox = i * (mob.radius * 0.26);
                ctx.beginPath(); ctx.ellipse(ox,0,mob.radius*0.42,mob.radius*0.28,0,0,Math.PI*2); ctx.fill();
            }
            ctx.restore();
            // glossy highlight
            ctx.fillStyle = 'rgba(255,255,255,0.2)'; ctx.beginPath(); ctx.ellipse(-mob.radius*0.22,-mob.radius*0.18,mob.radius*0.46,mob.radius*0.26,0,0,Math.PI*2); ctx.fill();
            // antennae (front, graceful curves)
            ctx.strokeStyle='#111'; ctx.lineWidth=Math.max(1.2, mob.radius*0.07);
            ctx.beginPath(); ctx.moveTo(-mob.radius*0.35,-mob.radius*0.62); ctx.quadraticCurveTo(-mob.radius*0.65,-mob.radius*1.1, -mob.radius*1.0,-mob.radius*1.5); ctx.stroke();
            ctx.beginPath(); ctx.moveTo(mob.radius*0.08,-mob.radius*0.62); ctx.quadraticCurveTo(mob.radius*0.42,-mob.radius*1.05, mob.radius*0.85,-mob.radius*1.45); ctx.stroke();
            // wings: translucent with soft rim
            ctx.fillStyle = 'rgba(255,255,255,0.28)'; ctx.strokeStyle = 'rgba(0,0,0,0.15)'; ctx.lineWidth = 1;
            ctx.beginPath(); ctx.ellipse(mob.radius*0.9, -mob.radius*0.32, mob.radius*1.0, mob.radius*0.44, -0.35, 0, Math.PI*2); ctx.fill(); ctx.stroke();
            ctx.beginPath(); ctx.ellipse(mob.radius*1.2, mob.radius*0.18, mob.radius*1.0, mob.radius*0.44, -0.25, 0, Math.PI*2); ctx.fill(); ctx.stroke();
            // stinger (back, darker triangular tip with gradient)
            const g = ctx.createLinearGradient(mob.radius*1.02, -mob.radius*0.45, mob.radius*1.6, 0);
            g.addColorStop(0, '#222'); g.addColorStop(1, '#0b0b0b'); ctx.fillStyle = g;
            ctx.beginPath(); ctx.moveTo(mob.radius*1.02,0); ctx.lineTo(mob.radius*1.6, -mob.radius*0.45); ctx.lineTo(mob.radius*1.6, mob.radius*0.45); ctx.closePath(); ctx.fill();
            // small eye
            ctx.fillStyle = '#0b0b0b'; ctx.beginPath(); ctx.ellipse(-mob.radius*0.18, -mob.radius*0.12, Math.max(1, mob.radius*0.18), Math.max(1, mob.radius*0.12), 0, 0, Math.PI*2); ctx.fill();
            ctx.fillStyle = 'rgba(255,255,255,0.6)'; ctx.beginPath(); ctx.ellipse(-mob.radius*0.12, -mob.radius*0.14, Math.max(0.6, mob.radius*0.06), Math.max(0.6, mob.radius*0.04), 0, 0, Math.PI*2); ctx.fill();
            ctx.restore();
        } else {
            const t = (mob.type || mob.name || '').toString().toLowerCase();
            ctx.save();
            if(t.includes('ant') && (t.includes('baby') || t.includes('baby-ant'))){
                ...
                bg.addColorStop(0, '#fff3e6'); bg.addColorStop(0.4, '#ffd28a'); bg.addColorStop(1, '#ffb86b'); ctx.fillStyle = bg;
                ctx.beginPath(); ctx.ellipse(0,0,mob.radius*1.2,mob.radius*0.95,0,0,Math.PI*2); ctx.fill();
                // bold dark stripes
                ctx.fillStyle = '#211f1f'; for(let i=-1;i<=1;i++){ ctx.beginPath(); ctx.ellipse(-mob.radius*0.15 + i*(mob.radius*0.45), 0, mob.radius*0.42, mob.radius*0.36, 0, 0, Math.PI*2); ctx.fill(); }
                // wing highlight
                ...
                // dark rounded stripes for better silhouette
